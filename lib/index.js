...
const constants = {
  ...
  ScheduleName: 'monitoringSchedule',
        ...
        return lambda.putFunctionConcurrency(params).promise()
          .catch((err) => {
            ...
            if (err.code === 'ResourceNotFoundException') {
              throw new Error(`The function ${funcName} is not deployed`)
            } else {
              throw new Error(`Unexpected error setting concurrency to zero:\n\t${err.message}`)
            }
          })
      })
      .then(() => {
        console.log(`Set concurrency level for ${funcName} to zero.\n`)
        options._ = 'remove' // eslint-disable-line no-param-reassign
        process.argv[2] = 'remove'
        if (options.verbose) {
          console.log("updating options['_'] to 'remove'")
          console.log("updating process.argv[2] to 'remove'")
        }
        return module.exports.remove(options)
          .then(() => {
            ...
            console.log(`You can re-deploy lambda after ${deployTime.toLocaleTimeString('en-US')}`)
          })
      })
  },
  /**
  ...
  monitor: (options) => {
    ...
    return BbPromise.all(preRead)
      ...
      .then((input) => {
        ...
        let output = yaml.safeDump(augmented)
